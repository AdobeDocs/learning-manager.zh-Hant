...
description: 本檔案提供與FedRAMP建議一致的Adobe Learning Manager (ALM)安全性帳戶管理、設定和合規性功能的完整摘要。
jcr-language: en-us
...
# Adobe Learning Manager安全性建議

## Adobe Learning Manager中的自訂管理員或整合管理員才能操作哪些安全性相關設定，以及這些設定對安全性有何影響？

Adobe Learning Manager的兩種特殊許可權帳戶型別（自訂管理員和整合管理員）各自擁有一組已定義的安全性相關設定，且含有所記錄的含意。
...
* 具有「設定」存取權（也具有「使用者」實體存取權）的自訂管理員可以將完整的「管理員」角色指派給自己，有效地升級為完整的管理控制。
...
* 整合管理員可設定LTI整合。 LTI一經啟用即無法停用。 公開的LTI憑證允許未經授權存取外部LMS平台的課程內容。


## Adobe Learning Manager中高層管理及特殊許可權帳戶的安全預設值建議為何，其設定位置為何？

### 最上層管理員帳戶預設值（在首次布建時設定）：
...
* 外部使用者設定檔到期：建立時設定在每個外部設定檔上 — ALM管理員>使用者>外部。 沒有無限的外部設定檔。
...
Adobe Learning Manager沒有專用的比較控制面板，無法自動顯示目前設定及建議的安全預設值。 不過，三種平台功能可讓管理員以手動或以程式設計方式稽核及比較目前的組態。

### 自訂角色報表 — 目前授權的角色指派：

在ALM中，導覽至「管理員>使用者>自訂角色」 ，然後按一下「下載」 （右上方），匯出所有自訂角色、其許可權集及其範圍指派的CSV。

### ALM REST API — 程式化設定擷取：

* GET /account端點會傳回JSON格式的帳戶層級設定資料，並可使用管理員角色OAuth範圍存取。 客戶可以程式設計方式呼叫此端點。
* GET /users端點（具有include=role篩選器）會傳回所有使用者的目前角色指派，啟用自動偵測非預期角色指派的功能。 使用工作API進行大量使用者匯出。
...
### Admin Console稽核記錄 — 所有驗證和角色變更的CSV匯出：

* 在Adobe Admin Console中，導覽至&#x200B;**深入分析>記錄>稽核記錄**，視需要套用篩選器，然後按一下&#x200B;**匯出CSV**
* 匯出的CSV會記錄每個管理動作，包括：2FA執行變更、工作階段限制變更、管理員角色指派和移除、使用者刪除和產品權益變更 — 所有這一切都具有時間戳記和執行者身分。
* 資料會保留90天。 Global Admin Console使用者可以匯出所有組織的記錄檔。

### ALM REST API — 目前角色指派和帳戶設定的JSON匯出：

* `GET /account` — 傳回JSON中的帳戶層級設定，包括使用中的登入設定資料和帳戶中繼資料。
* `GET /users?include=role` — 傳回所有使用者，以及他們目前在JSON中指派的角色。
* `GET /userGroups` — 傳回與稽核自訂角色範圍相關的所有使用者群組及其成員資格。
* 所有API回應都是JSON (`vnd.api+json`)。 驗證使用具有管理員角色讀取/寫入範圍的OAuth 2.0。
...
如需詳細資訊，請參閱[Adobe Learning Manager — 應用程式開發人員手冊](https://experienceleague.adobe.com/zh-hant/docs/learning-manager/using/integration/developer-manual)。
...
* 應用程式是由整合管理員在&#x200B;**整合管理員>應用程式>註冊**&#x200B;註冊。 OAuth 2.0使用者端ID和密碼是按應用程式發行。
* 有六個可用的範圍。 對於安全性設定管理，需要&#x200B;**管理員角色讀取/寫入**&#x200B;存取權。 這會透過API授予應用程式與完整管理員的相同存取權。
* 存取權杖可透過標準OAuth授權代碼或使用者端憑證流程取得。\
  **基底URL：**\
...
* `GET /account` — 傳回帳戶層級組態，包括JSON格式的帳戶設定資料，包括：
   * `complianceLabelDefaultID`
   * `showComplianceLabel`
   * `custom_injections`

### Adobe使用者管理API (Admin Console層)

* Adobe使用者管理API (UMAPI)可讓您以程式設計方式存取Admin Console作業：
   * 使用者布建
   * 產品權益指派
   * 組織層次的系統管理員角色指派
* UMAPI與ALM REST API不同，在Adobe組織層級運作。 使用它來自動化Admin Console角色指派和使用者布建。

## Adobe Learning Manager是否以機器可讀的格式（例如OSCAL、JSON或YAML）發佈其安全設定指引（建議的預設值）？

Adobe Learning Manager目前未以機器可讀的格式發佈其安全設定指南。

## Adobe Learning Manager的安全設定指南是否可公開使用，不需要登入或特殊存取？

是。 Adobe Learning Manager的安全設定指南可在Adobe Experience League上公開取得。 不需要帳戶、登入或授權即可存取。

公開可用的專案：

* FRR-RSC-01：安全管理指南：全層管理帳戶的完整生命週期指引。
* FRR-RSC-02：管理安全性設定與影響 — 所有安全性相關設定、其影響，以及建議的預設值。
...
* Adobe會為每個Adobe Learning Manager更新發佈編號的發行說明（例如&#x200B;*Update 100*、*Update 99*）。
* 這些檔案發佈於&#x200B;**Experience League**&#x200B;和檔案：
   * 新功能
   * 現有設定的變更
   * API新增和移除
   * 聯結器變更
   * 已棄用的功能
* 每個發行說明都包含&#x200B;**API變更**&#x200B;的專屬區段，清單：
   * 新端點
   * 已修改的回應欄位
   * 淘汰
   * 這些都與安全性相關設定功能直接相關。
...
* 每個主要版本都有專屬的&#x200B;**「新增功能」**&#x200B;頁面，記錄具有內容的安全性相關新功能。
* 安全相關更新的範例包括：
   * 自訂角色許可權處理的變更
   * 為自訂角色新增CSV建立的許可權可見性
   * API速率限制變更
...
* Adobe維護專用的&#x200B;**API淘汰**&#x200B;頁面，其中列出所有已淘汰和已移除的ALM API端點，包括發生淘汰的發行版本。
* 與安全相關的淘汰範例包括：
   * `GET /users`端點排序和覆寫行為的變更
   * 通知、報告日期篩選需求
