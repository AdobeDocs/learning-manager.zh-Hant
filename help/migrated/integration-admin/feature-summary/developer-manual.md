...
若要安全存取 Adobe Learning Manager API，您必須使用 Adobe Learning Manager 的 OAuth 2.0 機制進行認證。 這個過程包括註冊你的應用程式、產生授權碼、將其兌換成刷新令牌，最後用刷新令牌取得存取權杖。
...
| loResourceGrade | 這涵蓋了使用者在所註冊學習對象的情境下，消費特定資源的結果。 它包含使用者在資源中花費的時間、進度百分比、通過/不及格狀態，以及使用者在任何相關測驗中獲得的分數等資訊。 |
