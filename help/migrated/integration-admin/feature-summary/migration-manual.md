...
為避免混淆，官方 CSV 規範中已移除訂單欄位。 如果你還有現有的腳本或工具能產生這欄，你可以放心地刪除它;這不會影響學習程式的建立或顯示方式。
...
規劃您的遷移專案非常重要。 建議你將專案拆分成多個衝刺，並清楚指出你想在每個衝刺中遷移的內容。 甚至可以在每個衝刺後做一些驗證，這樣可以對該衝刺中遷移的資料有信心，而不是在專案結束時一次性進行一次大型驗證。 在你開始使用 Sprint 作為遷移專案的一部分之前，你需要分別在 FTP 和 Box 伺服器上傳資料和內容 CSV 檔案。 如果你沒有自訂 FTP 和 Box 的帳號，也可以自行建立。
...
你可以拖放內容檔案，或是從桌面點擊 **[!UICONTROL Upload]** 並選取檔案。 如果你的內容檔案大小很大，上傳檔案可能會有些延遲。 根據檔案大小，上傳到你的 Box 帳號所需的時間會有所不同。
...
當學習者透過替代註冊或關係完成課程時，Adobe Learning Manager 會產生一個專用的 webhook 事件，與標準課程完成的 webhook 不同，讓整合能為替代完成套用不同的處理邏輯。 Webhook 事件也會產生，用於追溯完成與不完成，涵蓋課程狀態的歷史變更，包括由關係更新所驅動的，確保外部系統與學習者當前完成狀態同步。
...
## VILT 會話遷移至 Adobe Learning Manager {#migrationofviltsessiontoalm}

Adobe Learning Manager 支援透過 CSV 檔案批量遷移與更新虛擬講師主導的培訓（VILT）課程資料。 使用此工作流程設定實例開始日期、將學習路徑實例與課程實例關聯，並設定 Microsoft Teams、Adobe Connect 與 Zoom 的虛擬教室課程。

>[!NOTE]
>
>所有 `almCourseID` `almModuleID`遷移 CSV 檔案中的欄位 ID 現在都使用 alm 前綴，例如 。 此前綴取代了早期版本中使用的舊有 prime 前綴。

### 基於 CSV 的 VILT 會話遷移

Adobe Learning Manager 遷移讓管理員能使用結構化 CSV 檔案批量建立或更新學習內容。 你可以將這些 CSV 工作流程應用於遷移課程（從外部系統匯入的內容）和後期課程（直接在 ALM 作者應用程式中建立的內容）。

VILT 會話遷移涉及四個 CSV 檔案：

* **課程實例 CSV：** 建立或更新課程實例，包括開始日期
* **LP 實例 CSV：** 建立或更新學習路徑實例，包括開始日期
...
使用 **Course Instance CSV** 和 **LP Instance CSV** 來新增或更新實例的開始日期。 這適用於遷移建立的實例和 UI 建立的（後置）實例。

**課程實例 CSV 檔案：新增開始日期**

1. 打開你的課程實例 CSV 檔案。
2. 如果欄位尚未存在，請加入 `startDate` 。
3. 請以 YYYY-MM-DD 格式輸入每個實例列的開始日期。
4. 在欄位中填寫 `almCourseID` 你想更新課程的 ALM 課程 ID。
5. 透過遷移執行上傳 CSV 檔案。
...
2. 如果欄位尚未存在，請加入 `startDate` 。
3. 請以 YYYY-MM-DD 格式輸入每個實例列的開始日期。
4. 在欄位 `almLearningProgramID` 中填入 ALM 學習路徑 ID。
5. 透過遷移流程上傳 CSV。

>[!NOTE]
>
>該 `startDate` 欄位是可選的。 如果你包含它，該值必須早於 。`completionDate`如果列 `startDate` 比 晚 `completionDate` ，則會出現錯誤並出現在遷移過程中。

### 將學習路徑實例與課程實例關聯

使用 LP 到 Course Instance Association 的 CSV 將學習路徑實例連結到特定的課程實例。 此步驟對於屬於學習路徑的VILT課程是必須的。

1. 打開 LP 轉為 Course Instance Association 的 CSV 檔。
2. 對每一列，請填入以下欄位：a。 `almLearningProgramID` — ALM 學習路徑識別碼b。 `almLearningProgramInstanceID` — ALM 學習路徑實例識別碼c. `almCourseID` — ALM 課程編號d. `almCourseInstanceID` — ALM 課程實例 ID
3. 透過遷移執行上傳 CSV 檔案。

### 支援的關聯情境

並非所有遷移與改造資源的組合都被支援。 在建立你的 CSV 前，請先檢視下表。

| 學習路徑來源 | 課程實例來源 | 支援 |
|-----------------------------|-------------------------------|-----------|
| 遷徙 | 遷徙 | 是的 |
| 後期改裝（UI創建） | 後期改裝（UI創建） | 是的 |
| 遷徙 | 後期改裝（UI創建） | 不 |
| 後期改裝（UI創建） | 遷徙 | 不 |

>[!NOTE]
>
>如果你需要將後製學習路徑實例與遷移課程實例（或反之）關聯，請直接透過 ALM 作者應用程式將課程加入學習路徑，而非使用這個 CSV。

### 設定虛擬教室課程細節

使用 **Session CSV** 建立或更新包含虛擬教室會議細節的 VILT 會議。 為支援此功能，會話 CSV 新增了四個欄位：

| 柱狀 | 說明 |
|--------------|-------------------------------------------------------|
| `almCourseID ` | 課程的ALM ID |
| `almModuleID` | 模組的 ALM ID |
| `metadata` | 包含 VC 系統專屬配置的 JSON 物件 |
| `meetingID` | 外部 VC 系統的會議 ID |

### 會議系統下的元資料格式

該 `metadata` 欄位接受 JSON 物件。 結構依會議系統而異。 所有鍵名皆為 **大小寫區分，必須完全如圖所示使用 camelCase** 。
...
所有 Teams 的元資料欄位皆為可選。 如果你沒有提供 `organizerEmail`，ALM 會使用你 ALM 帳號中設定的 Teams 管理員電子郵件作為預設的組織者。
...
Adobe Connect 課程必須&#x200B;**使用此`primaryInstructor`欄位**。其他欄位皆為可選。 你可以提供其中一種 `persistentRoom` ，或者 `templateID`如果你提供 `templateID`，ALM 會用該範本建立房間。

**Zoom**

Zoom 不需要 JSON 的元資料物件。 請使用會議 CSV 中的標準講師欄位通過該課程講師。

### 上傳 Session CSV

1. 打開你的 Session CSV 檔案。
2. 新增四個欄位：almCourseID、almModuleID、metadata 和 meetingID。
3. 在每個會話列中，將課程與模組的 ALM ID 填充 almCourseID 與 almModuleID。
4. 從你的風險投資系統（Teams、Adobe Connect 或 Zoom）新增 meetingID。
5. 請使用你會議系統的格式來建立元資料 JSON 物件。
6. 確保所有 JSON 鍵名稱都使用完全相同的 camelCase 拼法。 外殼不正確會導致排失敗。
7. 透過遷移執行上傳 CSV 檔案。
...
| 子嗣 | 解法 |
|-------|----------|
| 錯誤顯示「完成截止日期應大於開始日期」 | Ensure `startDate` 比 CSV 的實例更早 `completionDate` 。 |
| LP 與 course 實例關聯失敗 | 確認學習路徑和課程實例都是透過同一來源建立的（無論是遷移還是兩者都經過後推）。 不支援混合來源。 |
| 會話列因元資料錯誤失敗 | 檢查欄位中 `metadata` 所有 JSON 鍵名是否都使用 exact camelCase。 按鍵是大小寫區分的。 |
| Teams `isCompletionCriteria` 沒有影響 | Teams 的完成條件功能旗標必須由你的 ALM 帳戶管理員啟用，遷移值才會生效。 |
| 已建立 session 列，但 instructor 欄位是空的 | 若提供的講師電子郵件與 ALM 使用者不符，該課程將建立空置的講師欄位。 上傳前請確認該講師的電子郵件是否存在於ALM中。 |

## 遷移LTI模組 {#migrationofltimodules}

### 概觀

LTI 遷移擴展了現有的遷移工作流程，且不需額外的遷移檔案。 現有的課程、模組及模組關聯紀錄仍維持標準遷移格式。 LTI 專屬資訊透過模組版本資料提供。

### 使用檔案進行 LTI 遷移

LTI 模組的遷移是透過標準的遷移檔案進行的。
...
這些檔案中不需特定 LTI 欄位。 LTI 專屬設定已在檔案中 `module_version.csv` 設定。

### 設定 LTI 模組版本

使用該 `module_version.csv` 檔案來定義 LTI 模組版本的屬性。

除了現有支援的 `module_version.csv`欄位外，Adobe Learning Manager 也支援 LTI 專屬的值與屬性。

#### 內容類型

利用欄位中的`contentType`值`LTI`來識別模組版本為 LTI 模組。
...
當學習者在 Adobe Learning Manager 啟動模組時，學習者會被重新導向已設定的 LTI 端點。

*用於指定外部 LTI 提供者啟動 URL 的欄位*

| **場地** | **描述** |
|--------------|--------------------------------------------------|
| ltiLaunchUrl | 啟動網址由外部 LTI 平台提供 |

#### ltiCustomParams

指定自訂啟動參數，並在啟動時傳遞給 LTI 提供者。

當外部平台需要額外的啟動上下文或設定參數時，使用此欄位。

*該欄位用於將自訂啟動參數傳遞給 LTI 提供者*

| **場地** | **描述** |
|-----------------|------------------------------------------------------------|
| ltiCustomParams | 發布時會傳送給 LTI 平台的自訂參數 |

#### tpName

指定與該模組相關的第三方 LTI 供應商名稱。

*用來識別第三方 LTI 提供者的欄位*

| **場地** | **描述** |
|-----------|-----------------------------------------------------------------|
| tpName | 與模組相關的第三方LTI供應商名稱 |

### LTI 模組範例版本

以下範例展示了為 LTI 模組配置的模組版本記錄：
...
* 模組版本透過數 `contentType=LTI` 值識別為 LTI 模組。
* 啟動網址指向外部 LTI 提供者。
* 自訂發射參數由 `ltiCustomParams`提供。
* 提供者會透過欄位 `tpName` 被識別。
...
1. 建立課程記錄。`course.csv`
2. 建立模組記錄。`module.csv`
3. 將課程與模組關聯於 `course_module.csv`。
4. 在 中加入模組版本細節 `module_version.csv`。
5. 將值 `contentType` 設為 `LTI`。
6. 提供 LTI 啟動網址及任何可選的啟動參數。
7. 執行遷移衝刺。

遷移框架將 LTI 模組作為標準遷移工作流程的一部分處理。
