...
現在，選取設定檔，例如，使用者的標準設定檔，然後按一下該設定檔。 按一下 **[!UICONTROL Edit]** 和 **自訂應用程式設定** 區段，啟用核取方塊 **Adobe Learning Manager**. 這可讓使用者存取應用程式。
...
使用訓練資料存取聯結器將您的Adobe Learning Manager帳戶與資料儲存和擷取服務以及搜尋啟用系統整合，以啟用您的AEM Sites式介面來擷取訓練資料、轉譯網頁，並為學習者提供最佳化的訓練搜尋功能。

將訓練中繼資料從Adobe Learning Manager匯出至資料擷取與搜尋啟用服務。 您也可以建立排程來自動化這些匯出。
...
>[!NOTE]
>
>在2024年3月發行的Adobe Learning Manager中，我們已推出支援 [Adobe Commerce 2.4.6](https://experienceleague.adobe.com/docs/commerce-operations/release/notes/adobe-commerce/2-4-6.html?lang=en).
