...
+++更新103： Adobe Learning Manager 2025年11月發行
...
Experience Builder現在可供所有Adobe Learning Manager帳戶使用，讓客戶能使用低程式碼元件和彈性版面配置建立完整品牌的學習體驗。 Experience Builder可讓管理員建立自訂學習入口網站和頁面，且完全回應行動需求。

對於現有帳戶，Experience Builder更新將會自動推出。 繼續使用舊版UI或傳統UI的帳戶無法使用Experience Builder功能。

檢視[Experience Builder](/help/migrated/administrators/feature-summary/experience-builder/overview.md)以取得詳細資訊。

### Adobe Learning Manager行動網路介面

行動版網頁介面已在現代程式碼基底上重新建置，以加快載入時間、改善回應速度，並讓學習者在不同裝置上獲得更直覺的體驗。

依預設，所有新帳戶都會啟用更新的行動網站體驗。 除了啟用「社交學習」外，現有帳戶也會自動接收升級。 在這些情況下，先前的行動體驗將會維持作用中，直到社交學習移轉至新的相容架構為止。

>[!NOTE]
>
>新的Adobe Learning Manager行動網路介面不支援社交學習。

### 流體播放器增強功能

之前，流體播放器中的「下一步」按鈕未顯示即將來臨的模組的標題，因此學習者很難預測其學習歷程的下一步。

透過此增強功能，「下一步」按鈕現在包含下一個模組的標題，讓學習者能夠：
...
_流體播放器現在會顯示[下一步]按鈕，並附上即將推出的模組標題_

### L1意見回饋表單

L1意見反應只能在個別學習物件中設定，這會限制靈活性，並阻礙管理員在目錄中使用不同的表單。 管理員現在可以建立和使用多個L1意見表單，每個意見表單都可以指派給特定課程、學習路徑和認證。 表單現在支援多種問題型別，包括課程成效、NPS、李克特量表和自由文字問題。

意見表單可以翻譯成其他語言，同時在所有區域設定中保持相同的問題結構。 報表現在包含意見表單名稱和意見表單版本，以協助識別每個回應擷取的表單。

**使用案例**

* 指派量身打造的意見回饋表單給不同的訓練計畫。 例如，銷售課程可以使用以NPS為中心的問題，而合規性課程可以使用課程效果和Likert規模的問題來衡量清晰度和政策理解。
* 根據學習模組使用獨立的意見回饋表單。 例如，混合模組、自控進度模組和教室模組的三種不同形式。

檢視[L1意見表單](/help/migrated/administrators/feature-summary/l1-feedback-form.md)以取得詳細資訊。
...
在學習者成績單中，在時間欄中，分鐘會四捨五入至最接近的分鐘，而秒的值為00。 如需詳細資訊，請參閱&#x200B;[***時間欄***](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)。
...
* 下載已刪除使用者的學習者成績單。 如需詳細資訊，請參閱&#x200B;[***學習者成績單***](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)。
* 支援下列語言：
...
Learning Manager提供您為組織的Learning Manager學習者產生成績單的功能。 如需詳細資訊，請參閱[學習者成績單功能說明內容](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)。
