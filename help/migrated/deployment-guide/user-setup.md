---
description: 將使用者分類為內部和外部使用者。 內部使用者是指屬於特定組織或群組的使用者。
jcr-language: en_us
...
在包括Learning Manager在內的任何LMS中，管理使用者都是重要一環。 Learning Manager可讓您將使用者分類為內部和外部使用者。 內部使用者是指屬於特定組織或群組的使用者。 一般而言，企業內的使用者是內部使用者。 這些使用者擁有由管理員或管理員所指派的特定學習物件，並具有特定的截止日期。
...
作為管理員，您可能想要將外部使用者（例如合作夥伴組織的員工）新增到您的Learning Manager帳戶。 若要新增外部使用者：
...
1. 新增外部設定檔時，它們會顯示在&#x200B;**[!UICONTROL **&#x200B;外部使用者頁面&#x200B;**（**&#x200B;管理員&#x200B;**>**&#x200B;使用者&#x200B;**>**&#x200B;外部&#x200B;**）。]** 也會對這些使用者顯示名額限制、到期日和登入要求。
...
* **作用中狀態** — 在此狀態中，外部使用者可以註冊到Learning Manager應用程式，也可以登入應用程式。
...
除了大量新增內部使用者外，您也可以透過自行註冊來新增使用者。 您可以使用自助註冊，讓員工在您的Learning Manager帳戶中註冊成為學習者。 當您建立自助註冊設定檔時，會建立唯一的URL。 與員工共用此URL，讓他們能在Learning Manager中註冊。
...
您也可以使用Learning Manager中選取的使用者清單來建立自訂群組。 自訂群組可讓您指定使用者群組的名稱、說明和屬性。 您在Learning Manager中建立的自訂群組本質上為動態群組。 也就是說，如果新使用者新增了類似的屬性，就會自動新增到這些使用者群組中。
