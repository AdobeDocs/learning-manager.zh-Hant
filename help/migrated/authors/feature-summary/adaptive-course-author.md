...
當包含自適應課程的目錄外部分享給同儕帳號時，會適用以下行為：

* **直接分享的適應性課程：** 適應性課程不包含在共享目錄中。 這些資料不會出現在收款帳戶中。
* **學習路徑或認證中的自適應課程：** 若共享包含自適應課程的LP或認證，LP或認證本身會複製至接收帳戶。 其中的自適應路徑會被複製為 **一般路徑** ——自適應配置，包括所有可見性與完成規則，則不會被複製。 接收帳號中的作者將課程視為一般課程，所有模組對所有學習者皆可見。
* **自適應課程作為先修條件：** 若自適應性課程被設定為一般課程、學習路徑或認證的先修條件，該先決條件關係不會傳遞至接收帳戶。 家長課程或 LO 會直接進入接收帳戶，且沒有先決條件。

>[!NOTE]
>
>由於目錄共享時不會複製自適應配置，故在外部分享目錄前，請先審查所有前提關係及LP/認證結構。 接收帳號的學習者不會體驗到與來源帳號學習者相同的適應性行為。
...
>[!NOTE]
>
>**有序學習路徑：** 當自適應課程被納入有序學習路徑時，若課程中沒有明顯模組的學習者無法完成該課程。 這會阻擋有序學習路徑中所有後續項目的可存取性。 確保每位註冊學習路徑的學習者至少屬於一個使用者群組，該群組至少能在路徑中的每個適應性課程中顯示至少一個模組。

>[!NOTE]
>
>**一般學習路徑 — 自動退選：** 當學習者因使用者群組變更移除所有可見模組而自動退名於一般學習路徑中的自適應課程時，父學習路徑仍維持已註冊狀態。 學習路徑不會自動退選。 學習者即使已無法使用適應性課程，仍會在成績單中看到學習路徑已註冊。 如果你的使用情境需要學習路徑在自適應課程退名時也要退名，請使用 **自適應學習路徑** 而非一般學習路徑。
...
在一般課程中，講師可以設定 **候補名單上限**，限制每堂課可被列入候補名單的學習人數。

在自適應課程中， **候補名單限制** 設定在講師應用程式中是關閉的，且無法設定。 適應性課程中，候補學習者的人數沒有上限。 所有嘗試在課程滿額時報名的學習者都會被列入候補名單，且無限制。
...
>[!NOTE]
>
>**彈性學習路徑行為：** 當適應性課程屬於彈性學習路徑時，候補名單行為與直接選課不同。 若學習者在 Flex LP 中選擇自適應課程實例，且該實例無名額，該實例將被列入候補名單。 此情境的候補學習者資訊僅 **能從管理員> [自適應課程] >候補名單** 中看到——不會出現在 **管理員>學習路徑**&#x200B;中。 請查看自適應課程的候補名單分頁，管理透過彈性有限期（Flex LP）被列入候補名單的學習者。

當您下載 **彈性學習路徑中自適應課程的出席報告 PDF** 時，候補學習者會出現在 **PDF 的「活躍** 」區塊中。 這是因為學習路徑介面沒有獨立的候補名單區塊。 使用 **管理員> [自適應課程] 候補名單** >識別哪些學習者被列入候補名單，並在標記出勤前將其與已確認的參加者區分開來。
