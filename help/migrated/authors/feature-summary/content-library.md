...
1. 以作者身分登入後，在左窗格中按一下「內容資料庫」 ，然後按一下「新增」 。

   或者，您也可以按一下「快速入門」頁面中的「建立內容」 。

1. 在名稱欄位中，指定您要上傳之內容的名稱。
1. 在「描述」欄位中，輸入內容的描述。 請確定您要輸入的說明有意義。 字元限製為400個字元。
...
1. 在「持續時間」欄位中，您可以選擇輸入學習者要在此單元中花費的預期時間。 持續時間以分鐘為單位。

   如果學習者將課程標示為完成，則學習者所花費的學習時間會根據指定的期間計算。 如果學習者取用播放器中的內容，則播放器中的逗留時間會新增至學習時間。 如果實際內容時間小於指定的持續時間，則播放器一律遵循顯示的內容時間時，不會發生任何狀況。

1. 在「內容標籤」欄位中，輸入已上傳內容的標籤，以便可發現您的內容。
...
1. Learning Manager支援在Learning Manager中上傳影片內容的字幕。 現在，作者可以上傳包含註解的檔案以及視訊檔案。
   ...
   身為作者，當您上傳視訊或音訊內容時，也可以上傳包含註解的.vtt檔案。
...
使用「內容資料庫」頁面上的全新測驗建立工具，在Adobe Learning Manager中建立測驗。 建立的評估會成為內容庫的一部分，並可新增到「公用」資料夾以進行課程重複使用。
