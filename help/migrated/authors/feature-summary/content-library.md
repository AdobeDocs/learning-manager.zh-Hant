 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    <p><b>重要資訊：學習助理與基於 SCORM 的評量</b></p>
    <p>如果你的帳號啟用了學習助理，並且你將 SCORM / xAPI 套件上傳到設定為學習助理內容來源的目錄中，請注意以下事項：</p>
    <ul>
      <li>學習者可以使用學習助理（Learner Assistant）來詢問這些套件內容的問題。</li>
      <li>助理可能會產生解釋或摘要，有效揭示 SCORM 課程中小考題目的答案（例如，透過重述回饋或解釋「正確」選項）。</li>
   </ul>
   <p>對於高風險或監考評量（如合規或認證考試），Adobe 建議：</p>
   <ul>
      <li>將評量重的 SCORM 內容保留在未設定為學習助理來源的目錄中，或</li>
      <li>設計評量時，不會僅僅改寫學習內容就能輕易揭露正確答案。</li>
   </ul>
   <p>這種行為是預期中的：Adobe Learning Manager 並不會阻止 Learner Assistant 解釋你明確選擇為 AI 來源的目錄中的內容。</p>
    </td>
   <td>
    <p>零</p></td>
  </tr>
...
## 預覽課程內容 {#previewcontentlibrary}

課程內容加入內容庫後，您可以預覽。 要預覽，請選擇各課程列末尾的眼睛圖示。
...
1. 前往內容庫並選擇 [!UICONTROL Add Content]。
2. 上傳你的 MP3 或 MP4 檔案。
...
上傳新內容時，你可以選擇要存放在哪個資料夾。

1. 以作者身份登入，並前往 **管理** > **內容庫**，或從首頁選擇 **「建立內容** 」。
...
5. 在「新增資料夾」中&#x200B;**，選擇**「瀏覽資料夾」**，然後導覽到你想存放內容的資料夾。選擇資料夾以**&#x200B;確認。   ![](assets/cl3.png)
...
在內容庫中 **，每個內容檔案會**&#x200B;顯示其完整的資料夾路徑，例如銷售 **訓練** > **自學模組** > **PDF 資產**。 這條路徑會精確告訴你檔案在階層結構中的位置。

* 如果檔案存在於多個資料夾中，所有路徑都會以逗號分隔。

* 若路徑較長，則從一開始以省略號截斷（...），且最深的資料夾名稱始終顯示。

* 已發表、退休及草稿內容皆會出現在內容庫中，並顯示其資料夾路徑。
...
>[!NOTE]
>
>你無法將內容從私人資料夾移到公共資料夾，或從公共資料夾移到私人資料夾。
...
複製時會建立連結到額外資料夾的內容，且不會將其從當前位置移除。 內容檔案本身沒有重複。

1. 在內容庫&#x200B;**中**，選擇你想複製的每個內容檔案旁的勾選框。
2. 選擇 **動作** > **組織內容** > **複製到資料夾**。 **「複製內容到資料夾**」面板會打開，顯示資料夾階層結構。
3. 選擇你想讓內容出現的目標資料夾或資料夾。
4. 選擇 **複製內容**。 內容庫會更新，顯示已複製內容的新資料夾路徑。

>[!NOTE]
>
>你不能將內容從私人資料夾複製到公共資料夾，也不能從公共資料夾複製到私人資料夾。
...
在課程中新增自學模組時，你可以瀏覽資料夾層級，找到正確的內容檔案，而不必用名稱搜尋。

1. 以作者身份登入，並於左側導覽中進入 **課程** 。

2. 選擇 **新增** 以建立新課程或在編輯模式開啟現有課程。

3. 請前往 **模組** 區塊，選擇 **內容** > **新增模組**。

4. 在「 **選擇模組類型** 」對話框中，選擇 **「自學」** 作為模組類型。 **自學模組**&#x200B;對話框隨即開啟。

5. 選擇 **瀏覽資料夾** 以開啟資料夾階層。

6. 在資料夾層級中瀏覽，找到你想新增的模組。 選擇內容檔案。
...
若學習者已啟動課程但測驗未通過，則課程將被標記為已完成，但只有在測驗通過時才會達到成功標準。
...
在 Adobe Learning Manager 中，內容庫也會維護你上傳內容的版本。 如果你對內容做了任何更改，例如做了 PowerPoint 簡報，然後重新上傳到資料庫，版本號就會增加一。 這有助於你追蹤內容的變化。 新版本的內容可以同時套用到所有學習物件，或是針對每門課程分別進行個別更新。
+++
