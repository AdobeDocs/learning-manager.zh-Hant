...
# Adobe Learning Manager 內容撰寫器與 Adobe Learning Manager 如何協同運作

內容撰寫器負責撰寫。 Adobe Learning Manager 負責配送、註冊、追蹤與報告。 這兩個產品透過出版步驟相互連結。 一旦你從內容撰寫器發佈，課程就會成為 ALM 內容庫中的一個模組，可以組合成課程並指派給學習者。

## 內容撰寫者控制的範圍

- 課程與主題結構

- 課程內容 - 文字、圖片、影片、組件與知識檢查
...
## Adobe Learning Manager 控制什麼

- 學習者註冊與入學

- 模組元資料-時長、標籤、唯一 ID、到期

- 課程組合——將內容撰寫模組與其他學習內容結合
...
2. **發佈到 Adobe Learning Manager：** 完成創作後，透過 **匯出** 設定將內容作曲家連結到你的 ALM 帳號，然後發佈課程。 Content Composer 將課程作為符合 SCORM 標準的模組傳送至 ALM 內容庫。
   ![已發佈課程，並套用自訂標題、標誌和字型主題](../assets/49_published_course_custom_branding_header_updated.png)

3. **在 ALM 中設定模組：** 課程一旦發布，將以模組形式出現在 ALM 內容庫中。 ALM 作者會配置模組元資料——包括時長、標籤、唯一 ID 及到期設定——並將模組與其他學習內容一同加入 ALM 課程。
   ![模組元資料與完成標準欄位](../assets/50_alm_add_content_composer_module_metadata_updated.png)

>[!NOTE]
>
>如果你在 Adobe Learning Manager （ALM） 中設定完成與成功標準，這些設定會優先於內容撰寫器中定義的。
...
7.**使用版本管理**&#x200B;更新課程：當你在內容撰寫器中更新內容並重新發布時，ALM 會建立模組的新版本。 ALM 作者可以更新現有課程以使用最新版本。
