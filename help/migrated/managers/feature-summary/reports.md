...
報表會在控制面板中彙總。 報表必須存在於控制面板中。 A **預設儀表板** 預設會存在於報表頁面中。 您新增的任何報表都會移至此預設儀表板。 若要新增報表至個別儀表板，請使用下拉式箭頭並選擇「新增報表」 。 如需建立控制面板的詳細資訊，請參閱本頁面的控制面板區段。
...
**主要Y軸** 從下拉式選項為您的報表選擇第一個/主要條件。 對於某些選取的條件，您可以選擇從相鄰的「狀態」下拉式方塊中選擇一或多個狀態。 例如，對於課程註冊統計主要標準，狀態可以是已完成、不完整、已註冊等。 主要範圍資料在報表中以長條圖表示。

**次要Y軸** 從下拉式選項中選擇報告的次要Y軸條件/範圍。 例如，在學習計畫註冊選項中，從相鄰的「狀態」下拉式選單中選擇一或多個狀態。 次要範圍資料會以折線圖的形式呈現。

**X軸** 從下拉式選項中為報表選擇適當的x軸條件。 如果選擇X軸作為日期，則可選擇依日、月、季和年將X軸條件分組。
...
**個人資料** 選擇員工的指定。 它有助於根據員工的設定檔/指定來檢視員工報表。 例如，電腦科學家、工程師等。
...
若要根據使用者群組產生報表，請選擇 **使用者群組** 下拉式選項清單中的X軸顯示（如下面的熒幕擷圖所示）。

![](assets/x-axis-reporting.png)

*產生使用者群組報表*

其他 **選取** 下拉式清單會顯示在X軸旁，內含您的帳戶可用的使用者群組清單。 在此下拉式清單中，您可以選取一或多個使用者群組。
...
在X軸中，選擇使用者屬性選項並選取屬性，從 **選取** 旁邊的下拉式清單。 若要根據這些屬性建立自訂的使用者群組報表，您還需要在篩選中選擇適當的使用者群組。
...
1. 提供控制面板的名稱和說明，然後按一下 **[!UICONTROL Save]**.
...
**編輯/移至展示板/建立複製/刪除/重新調整報表大小**&#x200B;按一下每個報表右上角的下拉箭頭，以檢視「編輯/移動到控制面板/建立副本/刪除/調整大小」下拉選項。

<!--![](assets/edit-options-dashboard-300x126.png)-->

**編輯** 修改資料時，若要返回初始值，請按一下重設。 修改值後，按一下「儲存」 。

**移至儀表板** 您可以將目前報告移動到另一個儀表板，該儀表板是從儀表板清單中選擇的。

**建立復本** 您可以將報告複製到相同或另一個儀表板，該儀表板是從儀表板清單中選擇的。

**刪除** 按一下「刪除」以移除報表。 刪除報表前，系統會顯示警告/確認訊息。

**調整大小** 您可以重新調整報表大小，分為1×1 （中）和2×2 （大） 。
