...
請注意，意見收集是匿名的，僅會對已啟用選項的使用者顯示 **共用使用資料** 透過「應用程式設定」中的設定。
...
行動網站回應式UI支援僅適用於帳戶 **僅設定為沈浸式版面**. 回應式UI為 **不適用於傳統版面**.
...
點選 ![](assets/hamburger.jpg) 功能表檢視您的設定檔、通知及帳戶設定。

此功能表也可讓您掃描QR碼，標籤課程的出席情況。

您只需要點選 **掃描QR碼** 選項並掃描程式碼。
...
1. 若要掃描二維碼，請點選「 」 ![](assets/hamburger.jpg) 在畫面的左上角。
1. 點選 **掃描QR碼**.

掃描QR功能可讓學習者根據學習管理員產生的QR碼，執行下列任一動作：

* **註冊：** 這可讓學習者透過掃描二維碼來註冊課程。
* **完成：** 這可讓學習者透過掃描QR碼將課程標示為已完成。
* **註冊與完成：** 這可讓學習者透過掃描二維碼來同時註冊和標籤完成。
* **標示出席：** 這可讓學習者透過掃描QR碼，標籤特定工作階段的出席情況。
...
唯有在管理員啟用後，裝置應用程式才能使用此功能 **社交學習** 用於帳戶。
...
若要建立貼文，請點選 ![新增圖示](/help/migrated/learners/feature-summary/assets/prime-add.png) 圖示加以更新。
...
## 支援的內容格式 {#supportedcontentformat}
...
在行動應用程式中，您無法建立展示板。 若要建立展示板，您必須以學習者身分登入網頁應用程式。 如需建立展示板的詳細資訊，請參閱社交學習，位置在： [Learning Manager](/help/migrated/learners/feature-summary/social-learning-web-user.md).
...
* 按讚  ![](assets/prime-like.png)  或不喜歡  ![](assets/prime-dislike.png)   貼文。
* 按讚  ![](assets/prime-like.png) 或不喜歡 ![](assets/prime-dislike.png) 評論。

## 在其他展示板中建立貼文 {#createapostinotherboards}

您也可以在其他展示板中建立貼文。

點選 **所有展示板** 而且您可以檢視其他學習者在展示板中執行的活動。
...
在Manager核准提交後，狀態會變更為 **已核准**，以綠色著色。

如果Manager拒絕提交，則狀態會變更為 **已拒絕**，以紅色著色。
...
* 您只能在狀態為「 」時提交檔案 **擱置中的提交與已拒絕**.
* 當狀態為時，您無法提交檔案 **未決核准和已核准**.
...
學習者開始課程後，學習者即可檢視狀態 **檔案提交** 與上傳檔案的連結一起擱置時，會在熒幕上顯示，如下方熒幕擷圖所示。
...
上傳檔案後，狀態會變更為 **未決核准。**. 然後，講師會檢視 [待處理提交](/help/migrated/instructors/feature-summary/learners.md) 和透過講師網頁應用程式核准或拒絕提交。
...
使用者可以按一下「 」切換回線上模式 **檢視線上內容** 綠色列上的按鈕。 當使用者切換到線上模式時，會同步處理裝置中的內容。
