...
>[!NOTE]
>
>當一門自適應課程被設定為另一門課程的先修課，而該母課程透過目錄分享分享給接收帳號時，該自適應先修課程並不會分享給接收帳號。 無論先修條件是直接在課程中設定，或透過更高層次的學習對象（如學習路徑或認證）設定，這都適用。
>
>在接收帳戶中，母課程可用，但缺乏適應性先修課程。 接收帳號的學習者不會受到缺少先修條件的影響，因為對於透過目錄共享而未包含先置條件的內容，先置條件依賴性並未被強制執行。
>
>不要將自適應課程設定為你打算對外分享內容的先決條件。
...
>[!NOTE]
>
>當下載 **彈性學習路徑中自適應課程的出席報告 PDF** 時，候補名單學習者會出現在 PDF 的「活躍」區塊中。 學習路徑介面沒有專門的候補名單區塊，因此 PDF 匯出時沒有獨立的候補名單選項。 為準確識別候補學習者，請在標記出席前查看 **行政> [適應性課程] >候補名單** 。

**候補名單報告中的「嵌入」**&#x200B;欄位會標示包含此適應性課程的彈性學習路徑實例作為組成部分。它會顯示學習路徑名稱和學習物件 ID。 此文並非用來展示個別學員的註冊路徑。 對於嵌套於子學習路徑中，而子學習路徑本身又屬於父學習路徑的自適應課程，本欄僅會出現直接父學習路徑。

當自適應課程是重複認證&#x200B;**的一部分**&#x200B;時，刷新完成僅適用於學習者在根認證週期中的註冊情況。後續的重複循環包含一個獨立的自適應路徑實例，且不受刷新影響。 參加重複循環的學習者不會看到模組更新或完成次數被回滾。 若貴組織在定期認證中使用自適應課程，請在觸發刷新完成前告知管理員此限制。
