...
每次查詢都會回傳格式化的表格或可下載的 CSV 檔案，並附有淺顯明的結果計算說明。
...
3. **結果**：代理人會根據結果的性質，以文字、項目符號或表格的形式呈現您的資料。 結果附有淺顯的摘要。 當結果包含50列或以下時，摘要會包含對資料的分析見解。 當結果超過50列時，摘要會提供欄位層級的統計資料。
...
在 Adobe Learning Manager 中使用 Insights Agent，查詢學習資料，並以文字、表格或可下載的 CSV 檔案形式取得結果。
...
>[!TIP]
>
>如果你想探索相關資料，可以開始一個新的查詢，將你從第一個查詢中學到的內容納入其中。 例如，在查看各區域的註冊總數後，開始新查詢，深入探討表現不佳的區域。
...
- 從具體的問題開始，而非廣泛的問題。 「北美用戶組安全訓練課程的完成率是多少？」 回傳比「顯示完成資料」更有價值的結果。
...
您的查詢品質直接影響 Insights Agent 回傳的結果品質。 一個良好格式的查詢包含三個要素：上下文，例如內容內容和學習者;範圍，例如狀態、時間範圍和使用者狀態;以及欄位，例如你想要的輸出欄位。 學習如何使用正確的術語、查詢結構及範例查詢作為起點。
...
提供內容錨點有助於客服人員知道該關注哪些學習項目。 你可以透過以下任一方式作為錨點：
...
| 「告訴我誰失敗了」 | 「顯示學員姓名、電子郵件、課程名稱及完成狀態，以示未通過的學員」 |
...
- 「北美用戶組安全訓練課程的完成率是多少？」
- 「請顯示所有合規課程依使用者群組的完成率。 請包含使用者群組名稱、總註冊人數、完成總數及完成百分比。」
...
- 「顯示所有符合規範類型認證的完成率，依部門分組。 請提供系所名稱、總註冊人數及完成率。」
- 「過去30天各地區的入學分布如何？」
