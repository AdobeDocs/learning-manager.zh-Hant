...
### 自動清除已刪除使用者{#auto-purge}

自動清除已刪除使用者的功能，是用來清除已在 ALM 中刪除的使用者的資料。 清除是在可設定的保留期之後進行，重點放在批量作業上，讓大型客戶帳戶能有效率地處理而不影響效能。

大量刪除流程每批可處理多達 10,000 名使用者。 此功能定位為一個背景、以可靠性為重點的服務，針對大量刪除。

作為管理員，你可以指定刪除被清除使用者的期限。 請參閱 [管理員設定](/help/migrated/administrators/feature-summary/settings.md) 以獲取更多資訊。

#### 它的用途：

* 在帳號層級提供可設定的自動清除功能給已刪除使用者
* 確保用戶在符合清除標準後，於24小時內被清除
* 支援每日最多 10,000 名使用者的大量刪除，且不影響系統效能
* 在這些操作執行期間，維持整體系統的響應性與資料庫健康
* 強制執行自動化資料保存管理，以維護GDPR義務

#### 它不具備的部分：

* 批量刪除流程只會以排程 cron 工作形式執行（不會按請求按需執行）
...
3. 往下捲動頁面到 **自動清除已刪除的使用者**。
   ...
   *自動清除選項*
   >[!NOTE]
   >
   >如果&#x200B;**沒有啟用自動清除已刪除使用者**，該區塊底部會顯示「未設定&#x200B;**」的訊息**。
4. 選擇 **編輯**。
5. 勾選 **啟用勾選框**。
6. 輸入清除生效的持續時間。
   >[!NOTE]
   >
   >最低價值應該是一年。 你也可以增加1。 不過，你不能輸入像是1.5年或2.5年這樣的數值。 如果您需要自訂值作為持續時間，請聯絡我們的客服。
7. 選擇 **儲存**。 ALM 會顯示詳細的確認訊息。
