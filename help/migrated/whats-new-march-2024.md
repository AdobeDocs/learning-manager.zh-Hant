...
>[!NOTE]
>
>請參閱此[網路研討會](https://learningmanager.adobe.com/app/learner?accountId=98632#/course/9212121)，深入瞭解此版本的新功能。
...
檢視[匯入技能](administrators/feature-summary/import-skills-external-sources.md)以瞭解更多資訊。
...
如果大量學習者註冊了課程（例如，超過50,000個），則不會顯示學習者清單。 您可以在&#x200B;*搜尋學習者*&#x200B;搜尋列中搜尋學習者，或選取搜尋列頂端的&#x200B;**下載**&#x200B;連結以下載學習者清單。
...
如果使用者人數超過（例如50,000），則會出現訊息，要求您下載資料，以便稍後進行更詳細的分析。 搜尋列現在顯眼並以&#x200B;*名稱、電子郵件的格式顯示使用者 | UUID*。
...
對於外部使用者，也會套用相同行為。 如果使用者人數很多，您可以下載使用者，也可以在搜尋後以&#x200B;*名稱，電子郵件的格式擷取使用者的詳細資訊 | UUID*。

#### 使用者清理頁面

在[使用者清理]頁面上，對於已刪除的使用者，我們已在&#x200B;**刪除日期**&#x200B;移除排序功能。 您只能對UUID排序。
...
在目前版本的Adobe Learning Manager中，如果大量使用者已註冊認證，則無法檢視未註冊的學習者，因為&#x200B;**狀態**&#x200B;下拉式清單已停用。

在這個版本的Adobe Learning Manager中，如果註冊的使用者人數很多，**狀態**&#x200B;下拉式清單只會顯示兩個選項 — **已註冊**&#x200B;和&#x200B;**已取消註冊**。 預設會選取&#x200B;**已註冊**&#x200B;選項。 如果您選取&#x200B;**取消註冊**，則會顯示取消註冊的學習者清單。

#### 使用者群組變更

若是使用者群組，若使用者群組中的使用者數少於50,000，則&#x200B;**狀態**&#x200B;下拉式清單會顯示所有選項 — 「已認證」、「已指派」和「即將到期」。

如果使用者群組中的使用者人數很多，根據新設計，**狀態**&#x200B;下拉式清單只會顯示兩個選項 — **已註冊**&#x200B;和&#x200B;**已取消註冊**。
...
搜尋任何使用者時，**下載學習者**&#x200B;和&#x200B;**匯出**&#x200B;選項會下載相同的報表。 同時，在搜尋使用者群組時，您現在可以從該使用者群組下載經過篩選的使用者。 搜尋使用者群組時，**下載學習者清單**&#x200B;會變更為&#x200B;**下載使用者群組的學習者清單** **匯出**&#x200B;選項會再次下載整個清單。
...
根據[系統管理員設定](administrators/feature-summary/classroom.md#classroom-settings)，您身為作者，可以[建立、修改和刪除位置](administrators/feature-summary/classroom.md#add-classroom-location)。
...
檢視[學習計畫](administrators/feature-summary/learning-plans.md)以取得詳細資訊。
...
建立檢查清單時，作者可以在強制檢查清單區段中選取&#x200B;**啟用**。 如此一來，若學習者未通過檢查清單，即無法在單元中繼續。 只有在通過檢查清單後，他們才能繼續。

檢查清單檢閱者（即講師或管理者）可接著檢查檢查清單的狀態。 稽核者也可以不按順序稽核學習者的檢查清單。

### 重新評估檢查清單

建立檢查清單時，作者可以在重新評估區段中選取&#x200B;**啟用**。 如此可讓經理或講師重新評估學習者，直到他們通過檢查清單為止。
...
這表示在建立檢查清單時，「重新評估」區段中未選取&#x200B;**啟用**&#x200B;核取方塊。 如果選取此核取方塊，您將會在「教師檢查清單」頁面上看到「檢視/重新評估」按鈕。
...
在舊版Adobe Learning Manager中，學習者在下列情況時沒有收到工作階段相關的電子郵件、工作階段詳細資訊更新、工作階段邀請和工作階段提醒：
...
* 已更新工作階段詳細資訊和工作階段邀請（適用於學習者和講師）
   * 對於未來的工作階段，**工作階段詳細資訊的電子郵件已更新**、**工作階段邀請**&#x200B;已註冊的學習者和目前的講師，將不再使用。 在過去的工作階段中，**工作階段詳細資訊的電子郵件已更新**&#x200B;而已註冊學習者和目前講師的&#x200B;**工作階段邀請**&#x200B;將保持不變。
* 提醒電子郵件（適用於管理員和學習者）
   * 對於未來的工作階段，只會傳送&#x200B;**工作階段提醒**&#x200B;電子郵件。
...
我們已將&#x200B;*Instance*&#x200B;和&#x200B;*Learner*&#x200B;欄新增至產品術語辭彙表。
...
在目前版本的Adobe Learning Manager中，學習者在註冊Connect VC工作階段時被標示為&#x200B;**未參加**，但不符合完成條件。
...
如需詳細資訊，請檢視[Adobe Learning Manager行動應用程式](white-label.md)中的白色標籤。
...
如果您執行移轉以更新現有的課程或學習計畫或認證，則課程或學習計畫或具有&#x200B;**uniqueLOId**&#x200B;的認證會新增至作者應用程式。

移轉時，您必須更新CSV中課程或學習計畫或認證的&#x200B;**uniqueLOId**&#x200B;值，即使它是選用欄。

如果在更新具有&#x200B;**uniqueLOId**&#x200B;的現有課程或學習計畫或憑證時，執行移轉之前未新增&#x200B;**uniqueLoId**&#x200B;欄，則移轉之後，**uniqueLOId**&#x200B;值將以NULL值覆寫。

>[!NOTE]
>
>欄位&#x200B;**uniqueLoId**&#x200B;不適用於工作輔助CSV。
...
從[移轉手冊](integration-admin/feature-summary/migration-manual.md#csv-specifications-and-sample-csvs)下載CSV。
...
「登入存取」報表僅提供過去五個季度的資料。 如果任何整合管理員要求隨選下載包含&#x200B;**登入存取**&#x200B;的整合匯出，Adobe Learning Manager會顯示錯誤訊息。 不過，對其他報表沒有影響。
...
在這個版本中，LO API包含引數&#x200B;*isChecklistMandatory*。 如果值為True，則檢查清單為必填。
...
如果您嘗試下載超過五個季度的&#x200B;**登入存取**&#x200B;報告，便會顯示錯誤訊息。 如果您嘗試下載超過六個月的&#x200B;**使用者稽核**&#x200B;報告，則會顯示類似的錯誤訊息。
...
* **取消註冊**&#x200B;按鈕在第一次註冊後未如預期顯示，但在重新整理後按鈕顯示。
...
* 在行動應用程式或沈浸式模式中，刪除或淘汰課程執行個體並選取下一個使用中執行個體後，會顯示&#x200B;**註冊興趣**&#x200B;按鈕，而非&#x200B;**註冊**。
